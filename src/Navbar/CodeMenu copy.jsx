...
export default function CodeMenu() {
    ...
    useEffect(() => {
        const fetchUserLog = async () => {
            try {
            const response = await axios.get(`http://192.168.253.134:5001/api/time-left/${userId}`);
...
useEffect(() => {
    ...
    const autoStartTimer = async () => {
      ...
      try {
        await fetch("http://192.168.253.134:5001/api/start", {
          method: "POST",
          headers: {
    ...
    const handleUnload = () => {
      ...
      if (sessionId && userId) {
        navigator.sendBeacon(
          `http://192.168.253.134:5001/api/pause/${userId}/${sessionId}/${timeLeft}`
        );
      }
    };
...
const handleLogout = async () => {
    ...
    const runScript = async () => {
        ...
        if(userRole === '3' || userRole === '4'){
            ...
            try {
                const response = await fetch('http://192.168.253.134:5001/api/run-a10l10-Assesment', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
